...
:: Command line arguments to use when launching fplayer from a file association
set fplayer_args=

:: Get fplayer-init.exe location
cd /D %~dp0\..
set fplayer_path=%cd%\fplayer-init.exe
if not exist "%fplayer_path%" call :die "fplayer-init.exe not found"

:: Get fplayer-icon.ico location
set icon_path=%~dp0fplayer-icon.ico
if not exist "%icon_path%" call :die "fplayer-icon.ico not found"

:: Register fplayer-init.exe under the "App Paths" key, so it can be found by
:: ShellExecute, the run command, the start menu, etc.
set app_paths_key=HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\fplayer-init.exe
call :reg add "%app_paths_key%" /d "%fplayer_path%" /f
call :reg add "%app_paths_key%" /v "UseUrl" /t REG_DWORD /d 1 /f

:: Register fplayer-init.exe under the "Applications" key to add some default verbs for
:: when fplayer is used from the "Open with" menu
set classes_root_key=HKLM\SOFTWARE\Classes
set app_key=%classes_root_key%\Applications\fplayer-init.exe
call :reg add "%app_key%" /v "FriendlyAppName" /d "fplayer-com" /f
call :add_verbs "%app_key%"

:: Add fplayer to the "Open with" list for all video and audio file types
call :reg add "%classes_root_key%\SystemFileAssociations\video\OpenWithList\fplayer-init.exe" /d "" /f
call :reg add "%classes_root_key%\SystemFileAssociations\audio\OpenWithList\fplayer-init.exe" /d "" /f

:: Add DVD AutoPlay handler
set autoplay_key=HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers
call :reg add "%classes_root_key%\io.fplayer.dvd\shell\play" /d "&Play" /f
call :reg add "%classes_root_key%\io.fplayer.dvd\shell\play\command" /d "\"%fplayer_path%\" %fplayer_args% dvd:// --dvd-device=\"%%%%L\"" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayDVDMovieOnArrival" /v "Action" /d "Play DVD movie" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayDVDMovieOnArrival" /v "DefaultIcon" /d "%fplayer_path%,0" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayDVDMovieOnArrival" /v "InvokeProgID" /d "io.fplayer.dvd" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayDVDMovieOnArrival" /v "InvokeVerb" /d "play" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayDVDMovieOnArrival" /v "Provider" /d "fplayer" /f
call :reg add "%autoplay_key%\EventHandlers\PlayDVDMovieOnArrival" /v "MpvPlayDVDMovieOnArrival" /f

:: Add Blu-ray AutoPlay handler
call :reg add "%classes_root_key%\io.fplayer.bluray\shell\play" /d "&Play" /f
call :reg add "%classes_root_key%\io.fplayer.bluray\shell\play\command" /d "\"%fplayer_path%\" %fplayer_args% bd:// --bluray-device=\"%%%%L\"" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayBluRayOnArrival" /v "Action" /d "Play Blu-ray movie" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayBluRayOnArrival" /v "DefaultIcon" /d "%fplayer_path%,0" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayBluRayOnArrival" /v "InvokeProgID" /d "io.fplayer.bluray" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayBluRayOnArrival" /v "InvokeVerb" /d "play" /f
call :reg add "%autoplay_key%\Handlers\MpvPlayBluRayOnArrival" /v "Provider" /d "fplayer" /f
call :reg add "%autoplay_key%\EventHandlers\PlayBluRayOnArrival" /v "MpvPlayBluRayOnArrival" /f

:: Add a capabilities key for fplayer, which is registered later on for use in the
:: "Default Programs" control panel
set capabilities_key=HKLM\SOFTWARE\Clients\Media\fplayer\Capabilities
call :reg add "%capabilities_key%" /v "ApplicationName" /d "fplayer" /f
call :reg add "%capabilities_key%" /v "ApplicationDescription" /d "fplayer media player" /f
...
call :add_type "video/mpeg"                       "video" "MPEG Video"                 ".mpeg" ".mpg" ".mpe" ".mpeg2" ".m1v" ".m2v" ".mp2v" ".fplayer" ".fplayer2" ".mod" ".tod"
...
call :reg add "HKLM\SOFTWARE\RegisteredApplications" /v "fplayer" /d "SOFTWARE\Clients\Media\fplayer\Capabilities" /f

:: Add start menu link
powershell "$s=(New-Object -COM WScript.Shell).CreateShortcut('%ProgramData%\Microsoft\Windows\Start Menu\Programs\fplayer.lnk');$s.TargetPath='%fplayer_path%';$s.Save()"

echo.
echo 安装成功^—^！
echo 现在可以打开控制面板指定以类型打开fplayer
echo.
if [%unattended%] == [yes] exit 0
<nul set /p =按任意键打开控制面板. . .
pause >nul
...
:ensure_admin
	...
	if errorlevel 1 (
		...
		echo 右键 fplayer-install-com.bat 然后选择 "以管理员身份运行".
        echo 或者直接运行目录下 install-all.bat
		call :die
	)
	goto :EOF
...
:ensure_vista
	ver | find "XP" >nul
	if not errorlevel 1 (
		echo 安装程序只能在Windows7以上运行
		echo XP及以下系统请直接右键媒体，选择 "打开方式..."
		call :die
	)
	goto :EOF
...
:add_verbs
	...
	:: Set open command
	call :reg add "%key%\shell\open\command" /d "\"%fplayer_path%\" %fplayer_args% \"%%%%L\"" /f

	:: Add "play" verb
	call :reg add "%key%\shell\play" /d "&Play" /f
	call :reg add "%key%\shell\play\command" /d "\"%fplayer_path%\" %fplayer_args% \"%%%%L\"" /f
...
:add_type
	...
	:: Add ProgId
	set prog_id=io.fplayer%extension%
